...
const Chat = () => {
  ...
  const handleAiResponse = async (userInput) => {
    ...
    try {
      const response = await fetch(`https://www.backendpy.thefirstweb.com/api/v1/chat/chat/`, {
        method: "POST",
  ...
  const animateTyping = (response, tab) => {
    ...
    if (!chatId) {
      ...
      try {
        const response = await fetch(`https://www.backendpy.thefirstweb.com/api/v1/chat/start`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        >
            ...
            {activeTab === "ai"
              ? aiMessages.map((message, index) => (
                  ...
                  >
                    {message.type === "ai" && (
                      <img
                        src="https://www.thefirstweb.com/robot.png"
                        alt="Admin"
          ...
          {activeTab === "ai" && (
            <div className="flex justify-center align-items text-[8px] mx-6 text-center my-2">
              UK25 Chatbo Chatbot (powered by GPT-4) is experimental and may provide
              inaccurate or misleading responses. User discretion is advised.
              The Himalayan Vidya Daan Trust cannot be held responsible for any consequences
